...
export const sessions = pgTable('sessions', { token: text('token').primaryKey(), userId: integer('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(), expiresAt: timestamp('expires_at').notNull() }, (t) => [index('sessions_user_expires').on(t.userId, t.expiresAt)]);
...
}, (t) => [index('listings_status_featured_id').on(t.status, t.featured, t.id), index('listings_owner_created').on(t.ownerId, t.createdAt)]);
...
}, (t) => [index('enquiries_listing_created').on(t.listingId, t.createdAt), index('enquiries_user_created').on(t.userId, t.createdAt)]);
export const reviews = pgTable('reviews', { id: serial('id').primaryKey(), listingId: integer('listing_id').references(() => listings.id).notNull(), userId: integer('user_id').references(() => users.id).notNull(), rating: integer('rating').notNull(), comment: text('comment').notNull(), createdAt: timestamp('created_at').defaultNow().notNull() }, (t) => [uniqueIndex('review_user_listing_unique').on(t.userId, t.listingId), index('reviews_listing_created').on(t.listingId, t.createdAt)]);
...
export const content = pgTable('content', { key: text('key').primaryKey(), value: text('value').notNull() }, (t) => [index('content_key').on(t.key)]);
export const rateLimits = pgTable('rate_limits', { key: text('key').primaryKey(), windowStart: timestamp('window_start').notNull(), count: integer('count').notNull().default(0) });
